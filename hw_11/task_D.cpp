...
int main() {
    ...
    std::string new_s = s + '#' + t;
    std::vector<int> suff = build_suff_array(new_s);
    std::vector<int> lcp = build_lcp(suff, new_s);

    int threshold = s.size() + 1;
    int max_substr_len = 0;
    int max_substr_idx = 0;

    for (int i = 2; i < suff.size(); i++) {
        if ((suff[i] < threshold and suff[i - 1] >= threshold) or (suff[i - 1] < threshold and suff[i] >= threshold)) {
            if (lcp[i - 1] > max_substr_len) {
                max_substr_len = lcp[i - 1];
                max_substr_idx = suff[i - 1];
            }
        }
    }

    std::cout << new_s.substr(max_substr_idx, max_substr_len) << '\n';
